...
##TO BE DELETED
#Model selection test (will need moved inside the sp.loop)
...
covs_df<-sp.data %>% select(scale.lake, per_forest, per_wetlg, per_wetsm, per_agri, per_urban, Roadkm, SolReacP, nitrateN, ammoniaN, cond)

for(i in 1:11){
  
  f1 <- as.formula(paste0("maxocc ~ -1 + 
  # cell ICAR random intercepts
  f(alpha_i, model=besag, graph=g1, constr=FALSE, scale.model=TRUE,
    hyper = list(prec = list(prior = prior, param = c(1, 0.01)))) +
  # cell ICAR random year slopes
  f(tau_i, std_yr, model=besag, graph=g1, constr=FALSE, scale.model=TRUE,
    hyper = list(prec = list(prior = prior, param = c(1, 0.01)))) +
  # random site intercepts
 ...
 # basin-year effect
  f(gamma_ij, model=iid, constr=TRUE, 
   hyper = list(prec = list(prior = prior, param = c(1, 0.01)))) +"
    , paste0(colnames(covs_df)[1:i], collapse = " + ")))

out1<- inla(f1, family="binomial", data=sp.data,
            control.compute=list(dic = TRUE, cpo=TRUE, waic = TRUE, return.marginals.predictor=TRUE, config=TRUE),
            control.inla=list(strategy="adaptive",
                              int.strategy="auto"),
           control.predictor = list(compute=TRUE, link=1),
           control.family=list(link='logit'),
...
model_selection <- if(i==1){rbind(c(model = paste(colnames(covs_df)[1:i]),waic = out1$waic$waic))}else{rbind(model_selection,c(model = paste(colnames(covs_df)[1:i],collapse = " + "),waic = out1$waic$waic))
  }
...
par(mfrow=c(3,4))
plot(out1$marginals.fixed[[1]], ty = "l", xlab = expression(beta[scale.lake]), ylab = "Density") 
plot(out1$marginals.fixed[[2]], ty = "l", xlab = expression(beta[per_forest]), ylab = "Density") 
plot(out1$marginals.fixed[[3]], ty = "l", xlab = expression(beta[per_wetlg]), ylab = "Density") 
plot(out1$marginals.fixed[[4]], ty = "l", xlab = expression(beta[per_wetsm]), ylab = "Density") 
plot(out1$marginals.fixed[[5]], ty = "l", xlab = expression(beta[per_agri]), ylab = "Density") 
plot(out1$marginals.fixed[[6]], ty = "l", xlab = expression(beta[per_urban]), ylab = "Density") 
plot(out1$marginals.fixed[[7]], ty = "l", xlab = expression(beta[per_Roadkm]), ylab = "Density") 
plot(out1$marginals.fixed[[8]], ty = "l", xlab = expression(beta[per_SolReacP]), ylab = "Density") 
plot(out1$marginals.fixed[[9]], ty = "l", xlab = expression(beta[per_nitrateN]), ylab = "Density") 
plot(out1$marginals.fixed[10], ty = "l", xlab = expression(beta[per_ammoniaN]), ylab = "Density") 
plot(out1$marginals.fixed[[11]], ty = "l", xlab = expression(beta[cond]), ylab = "Density") 
